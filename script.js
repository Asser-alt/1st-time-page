...
function updateOneTimePasswordDisplay() {
  const display = document.getElementById("oneTimePasswordDisplay");
  if (!display) return;
  display.textContent = generateOneTimePassword();
}
...
function setupHomePage() {
  ...
  updateOneTimePasswordDisplay();
  renderClients();
  setInterval(updateHomeReminderBanner, 1000);
  setInterval(updateOneTimePasswordDisplay, 1000 * 60); // update every minute
}
